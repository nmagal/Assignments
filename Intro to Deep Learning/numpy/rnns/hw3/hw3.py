...
class CharacterPredictor(object):
    ...
    def forward(self, x, h):
        ...
        output_h_gru = self.rnn(x,h)
        output_linear = self.projection(output_h_gru)
        
        return output_linear, output_h_gru
...
def inference(net, inputs):
    ...
    init_h = np.zeros(net.hidden_dim)    
    logits = np.zeros((seq_len,net.num_classes))
    
    for sequence in range(seq_len):
        logits[sequence, :], init_h = net.forward(inputs[sequence, :],init_h)
