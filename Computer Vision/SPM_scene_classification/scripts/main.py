...
def main():
    ...
    img = Image.open(img_path)
    img = np.array(img).astype(np.float32)/255
    dictionary = np.load(join(opts.out_dir, '../output/dictionary.npy'))
    ...
    print(conf)
    print(accuracy)
    np.savetxt(join(opts.out_dir, '../output/confmat.csv'), conf, fmt='%d', delimiter=',')
    np.savetxt(join(opts.out_dir, '../output/accuracy.txt'), [accuracy], fmt='%g')
